...
Defined in `proto/sample/v1/{sample.proto,partner.proto}`; Go stubs in `gen/go/sample/v1`. Two
services are **authenticated** (require a bearer token with scope `sample:user`) and two paths are
**public**.
