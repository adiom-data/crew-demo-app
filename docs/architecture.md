...
| BillingService impl | `internal/api/billing.go` | `CreateCheckoutSession` (bearer) + the public `POST /stripe/webhook` raw route. Owns the `stripe-go` client and subscription state writes. |
...
Defined in `proto/sample/v1/{sample.proto,partner.proto,billing.proto,agentquery.proto}`; Go stubs in
`gen/go/sample/v1`. Three services are **authenticated** (require a bearer token with scope
...
| `CreateCheckoutSession` | `sample.v1.BillingService` | bearer | Start a Stripe Checkout session for a partner (monthly/annual). |
...
| `POST /stripe/webhook` | raw HTTP route | **public** | Stripe events; authenticated by HMAC signature over the raw body, not a bearer token (INV-4c). |
...
- `web/src/api/{clients.ts,sampleClient.ts,partnerClient.ts,billingClient.ts}` — one shared `AuthTokenManager`; the
