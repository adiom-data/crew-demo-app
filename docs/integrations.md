...
| **Stripe** | `internal/api/billing.go` | `stripe-go/v86` client (`stripe.NewClient`) creates Checkout Sessions; Stripe calls back to the public `POST /stripe/webhook`. Optional — absent config disables billing. | `internal/api/billing.go` |
...
Admins subscribe a partner from partner detail: `sample.v1.BillingService/CreateCheckoutSession`
(authenticated) returns a Stripe Checkout URL; Stripe then posts `checkout.session.completed` and
`customer.subscription.updated|deleted` to the public `POST /stripe/webhook`, which writes the
partner's subscription columns and appends a `subscription` activity.

Every var is **optional**. When `STRIPE_SECRET_KEY`, `STRIPE_PRICE_MONTHLY` or `STRIPE_PRICE_ANNUAL`
is missing, the API still boots: the RPC returns `CodeUnavailable` and the webhook returns 503. This
keeps previews and local runs working without real Stripe credentials.

| Var | Default | Required | Source / meaning |
|-----|---------|----------|------------------|
| `STRIPE_SECRET_KEY` | — | no | secret `crew-demo-stripe` — `sk_test_…` / `sk_live_…`. |
| `STRIPE_WEBHOOK_SECRET` | — | no | secret `crew-demo-stripe` — `whsec_…`; required for the webhook to accept events. |
| `STRIPE_PRICE_MONTHLY` | — | no | secret `crew-demo-stripe` — `price_…` for the monthly plan. |
| `STRIPE_PRICE_ANNUAL` | — | no | secret `crew-demo-stripe` — `price_…` for the annual plan. |

The Checkout success/cancel URLs use `PUBLIC_BASE_URL` when set and otherwise fall back to the
request's `Origin` header, so no extra env is needed in release or preview. Do **not** set
`PUBLIC_BASE_URL` solely for billing — it also feeds the OIDC `/auth/callback` base.

Webhook notes:
- The signature is verified over the **raw** request body before any JSON decode.
- Verification passes `IgnoreAPIVersionMismatch: true`. `stripe-go` v86 pins
  `stripe.APIVersion = 2026-06-24.dahlia` and otherwise rejects events from a Stripe account on an
  older API release train.
- Handling is idempotent: a redelivered event that would not change state is a no-op, so retries
  don't duplicate activity rows.

Register the endpoint in the Stripe dashboard at `https://<host>/stripe/webhook` for
`checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`.
Locally, `stripe listen --forward-to localhost:8080/stripe/webhook` prints the `whsec_…` to use.
...
| `crew-demo-stripe` | `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_MONTHLY`, `STRIPE_PRICE_ANNUAL` | API Deployment (all `optional: true`) |
...
`crew-demo-stripe` is a **separate secret** from `crew-demo-auth` on purpose: billing credentials
rotate independently of the auth seeds, and the API tolerates it being absent. Create it with:

```sh
kubectl create secret generic crew-demo-stripe \
  --from-literal=STRIPE_SECRET_KEY=sk_test_… \
  --from-literal=STRIPE_WEBHOOK_SECRET=whsec_… \
  --from-literal=STRIPE_PRICE_MONTHLY=price_… \
  --from-literal=STRIPE_PRICE_ANNUAL=price_…
```
